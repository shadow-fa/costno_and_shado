...
if (isServer) then {
	...
	//add damage handler to objectives
	{
		_x addEventHandler ["HandleDamage",
		{
			...
			private _deadly_projectiles = ["DemoCharge_Remote_Ammo", "SatchelCharge_Remote_Ammo", "LightningBolt"];

			//if ((_projectile == "DemoCharge_Remote_Ammo" || _projectile == "SatchelCharge_Remote_Ammo" || _damage >= 1) && _projectile != "B_30mm_AP_Tracer_Red") exitWith {
			if (_projectile in _deadly_projectiles) exitWith {
				_object setDamage 1;
				(_object getVariable "marker_name") setMarkerType "mil_objective";
				(_object getVariable "marker_name") setMarkerColor "colorBlack";

				objectives_done set [_object getVariable "index", 1];
				publicVariable "objectives_done";

				//[["ObjectiveDestroyed", []], "BIS_fnc_showNotification", true, false] spawn BIS_fnc_MP;

				//converge on the objective after it's blown up
				//TODO check for locality issues. does everyone execute this?
				private _groups_near = (_object getVariable ["groups",[]]);
				{
					//let every 2nd group come
					if (_forEachIndex % 2 == 0) then {
						...
						//go near objective
						_wp = _x addWaypoint [(position _object) getPos [100,random 359], 0];
						_wp setWaypointBehaviour "AWARE";
						_wp setWaypointTimeout [1,2,4];
						_wp setWaypointCompletionRadius 20;
						//go to objective
						_wp = _x addWaypoint [(position _object) getPos [30, random 359], 0];
						_wp setWaypointType "SAD";
					};
				} forEach _groups_near;
				//return damage
				1
			};
			//return damage
...
if (isServer) then {
	[] spawn {
		...
		while {_ending == -1} do {
			//check endings
			if (count objectives == ({_x == 1} count objectives_done)) then {
				//success
				_ending = 1;
			};
			//everyone died
			if (0 == ({alive _x && (side _x == resistance)} count playableUnits)) then {
				if( ({_x == 1} count objectives_done) > 0) then {
					//partial success
					_ending = 2;
				}
				else{
